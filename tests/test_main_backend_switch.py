...
class TestSwitchOwnedLifecycle:
    ...
    def test_owned_lifecycle_uses_tws_calendar_service(
        self,
        store: InMemoryTokenStore,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """AP `2a203c58-...` Phase 5: BG_BACKEND=tws muss
        TWSCalendarService unter app.state.calendar_service haengen."""
        ...
        with TestClient(app):
            assert isinstance(
                app.state.calendar_service, TWSCalendarService
            )
            # Static-Mapping liefert immer eine nicht-leere Liste -
            # damit greift der 503-Pfad in /v1/exchanges nicht im
            # TWS-Modus.
            assert app.state.calendar_service.cached_exchanges

    def test_cp_backend_uses_cp_calendar_service(
        self,
        store: InMemoryTokenStore,
        cp_gateway_mock: Any,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Schwesterfall: BG_BACKEND=cp behaelt cp.CalendarService."""
