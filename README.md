...
**Status:** v1.28.0 — Service deployed auf cma-pi-1 (Port 4000), AP-01..AP-05 + AP-10 abgeschlossen, AP-11 Phase A+B vollständig (K1..K9). WS-Lifespan-Verdrahtung (BG_QUOTES_SOURCE=ws opt-in) ist drin; Live-Smoke gegen U25235077 als nächster Schritt nach Pi-Deploy. Quote-Volume-Bugfix (IBKR-Field 7762 ÷ 10⁶) in v1.25.0. iserver-Bridge-Health-Probe + Auto-Reauth bei `no bridge`-Drift in v1.26.0. ISIN-Filter auf `/v1/instruments/search` (Karte 12e04c98) inkl. optionalem MIC-Disambiguator in v1.27.0. Auto-Login-Skeleton für den Paper-Stack (Karte ece90a8e Phase A) — `BG_STACK_KIND`-Pflicht-Env, Hard-Guards (live + Auto-Login = Startup-Fail), `LifecycleSnapshot`-Auto-Login-Felder, Throttle-Logik (5/15/45 min Backoff, 5/Tag-Stop) und Health-Endpoint-Erweiterung in v1.28.0; der Headless-Sidecar selbst folgt in Phase B. Aktueller Architektur-Stand und Komponenten-Übersicht in [`docs/02-architecture.md`](docs/02-architecture.md). Vollständige Versionshistorie in [`CHANGELOG.md`](CHANGELOG.md).
