...
**Status:** v1.31.0 — Service deployed auf cma-pi-1 (Port 4000), AP-01..AP-05 + AP-10 abgeschlossen, AP-11 Phase A+B vollständig (K1..K9). WS-Lifespan-Verdrahtung (BG_QUOTES_SOURCE=ws opt-in) ist drin; Live-Smoke gegen U25235077 als nächster Schritt nach Pi-Deploy. Quote-Volume-Bugfix (IBKR-Field 7762 ÷ 10⁶) in v1.25.0. iserver-Bridge-Health-Probe + Auto-Reauth bei `no bridge`-Drift in v1.26.0. ISIN-Filter auf `/v1/instruments/search` (Karte 12e04c98) inkl. optionalem MIC-Disambiguator in v1.27.0. Auto-Login-Skeleton für den Paper-Stack (Karte ece90a8e Phase A) in v1.28.0. Auto-Login Phase B in v1.29.0 — Sidecar-Image (`ops/auto-login/`, Playwright/Chromium ARM64), echter `DockerSubprocessAutoLoginRunner`, Lifecycle-Verdrahtung (Trigger nach `AUTH_LOST`/`CP_DOWN`), `POST /v1/admin/auto-login/trigger`, Compose-Override für den Paper-Stack mit `docker.sock`-Mount und Pi-Setup-Runbook. v1.30.0: Auto-Login-PoC mit IBeam (Karte c824617e) zeigte ARM64-Browser-Stabilität, scheiterte aber am IBKR-Submit; deshalb wurde der manuelle Recovery-Login bequemer gemacht — cpgateway exponiert auf cma-pi-1 jetzt einen lokalen 127.0.0.1-Bind (Live `:5000`, Paper `:5001`), so dass ein Browser auf der Pi-Desktop-Session den Login direkt über `localhost` abwickelt (Runbook `docs/runbooks/cpgateway-pi-desktop-login.md`). v1.31.0: Cookie-Bridge (Karte 406fce15) — `httpx.AsyncClient` hat jetzt einen Cookie-Jar mit Path-Override-Hook (Set-Cookie mit `Path=/sso` wird automatisch auf `Path=/` umgeschrieben), und der neue Endpoint `POST /v1/internal/seed-cookies` (admin:*) übernimmt JSESSIONID + x-sess-uuid aus dem Pi-Browser-Profil in den Service-Jar, ruft optional `/iserver/auth/ssodh/init` mit `keepAlive=true` auf und triggert sofort einen Lifecycle-Tick. Damit ist der nsenter-Browser-Login (Karte 739777a9) end-to-end nutzbar, ohne dass der Service die Cookies aus dem Browser-Profil ziehen muss. Aktueller Architektur-Stand und Komponenten-Übersicht in [`docs/02-architecture.md`](docs/02-architecture.md). Vollständige Versionshistorie in [`CHANGELOG.md`](CHANGELOG.md).
