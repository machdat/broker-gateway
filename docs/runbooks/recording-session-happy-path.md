...
| `marketdata/snapshot` | First-Call-Prime (leer) → Second-Call (Werte) | Bodies kommen sofort mit Werten beim Polling-Aufruf, kein Prime-Verhalten. Felder: `conid` (int), `conidEx` (string), `_updated`, `6119`, `server_id`, `31/84/86`, `6509: "ZB"` (statt "DPB"), `6508` mit `serviceID*`-Liste | Service-Code muss Verhalten neu interpretieren — Prime ist offenbar nur bei initialer Subscription/Streaming, nicht bei Polling-Snapshot. Folgekarte fuer `cp/quotes.py`. |
...
5. **`cp/quotes.py`**: Snapshot-Verhalten klaeren — Prime nur bei initialer Subscription, nicht bei Polling. Availability-Code `ZB` aufnehmen.
