...
Response 200 (Stand v1.26.0, gegen Live verifiziert):
...
| `iserver_bridge_ok` | Letztes Resultat des iserver-Bridge-Probes via `GET /iserver/auth/status`: `true` wenn das Trio (`authenticated`/`established`/`connected`) gesund war, `false` bei Bridge-Drift, `null` solange noch kein Probe gelaufen ist (ab v1.26.0) |
| `last_bridge_probe_at` | UTC-Timestamp des letzten Bridge-Probes (ab v1.26.0) |
| `consecutive_bridge_failures` | Aufeinanderfolgende Bridge-Drift-Befunde; reset auf `0` nach erfolgreichem Probe oder Recovery (ab v1.26.0) |
...
| `BG_CP_BRIDGE_PROBE_INTERVAL_S` | `120` | iserver-Bridge-Probe-Periode in Sekunden (ab v1.26.0) |
